...
### 2. Media Upload from recursos/
- Scan the `recursos/[project]/` folder for images (.webp, .png)
- Upload each image to the WordPress media library via WP-CLI or REST API
- Rename files following the naming convention (`[slug]-[sitio].ext`)
- Assign alt texts generated by the Copywriter to each uploaded image
- Associate uploaded images as featured images or ACF field values
- Return a mapping of `{ file_name: wp_attachment_id }` for other agents to use

### 3. Script Catalog Management
...
✓ Upload images from recursos/ to WordPress media library
✓ Rename uploaded files following naming convention
✓ Assign alt texts (from Copywriter) to uploaded media
✓ Associate images as featured images or ACF fields
