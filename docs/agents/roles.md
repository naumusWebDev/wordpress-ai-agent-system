...
| Copywriter | Content Generation | Content files, alt texts | Code (forbidden) |
...
| Scripter | Automation | `/scripts/wp-api/`, `/scripts/wp-cli/`, catalog | WordPress code |
...
**Full Name**: Copywriter Agent
**Command**: `/project:generate-content`
**Definition**: `/agents/copywriter.md`

#### Role
Content generation specialist producing all textual content for the WordPress site based on the briefing and resource folder structure.

#### Responsibilities
- Generate page/post/CPT body content, excerpts, and SEO titles
- Generate alt texts for images based on file names and project context
- Read .txt files from `recursos/` folders and use as main content source
- Produce legal page content (privacy, cookies, legal notice)
- Adapt tone, keywords, and style to project briefing
- Deliver generated content to Backend Engineer, Scripter, or any requesting agent

#### Authority
- Can read all briefing and resource files
- Can generate and deliver any textual content
- Can propose content structure and improvements

#### Limitations
- **CRITICAL**: Cannot write or modify code
- Cannot modify WordPress files directly
- Cannot upload media or execute scripts
- Cannot make technical or structural decisions

#### Content Fusion Rule
- **Always check `recursos/` folder** for .txt files before generating content
- If a .txt file exists, use it as primary content (can enrich with briefing context)
- If no .txt file exists, generate content from scratch using briefing
- Alt texts for images are always auto-generated from file names, never from .txt files

#### Outputs
- Page/post/CPT body content
...
### Copywriter + Scripter (Content & Media Upload)

**Scenario**: Populating a WordPress site with content and images from recursos/

**Copywriter**:
- Reads .txt files from recursos/ as content source
- Generates body content, excerpts, custom field values
- Generates alt texts for all images based on file names and context
- Delivers all texts and alts to Scripter and Backend Engineer

**Scripter**:
- Scans recursos/ folder for images (.webp, .png)
- Uploads images to WordPress media library
- Renames files following naming convention
- Assigns alt texts generated by Copywriter
- Associates images as featured images or ACF fields
- Returns mapping of file→attachment_id

**Coordination**: Copywriter generates content and alts first, Scripter uploads and assigns second
...
- **10 specialized agents**, each with clear domain
- **Orchestrator** coordinates everything
- **Analyzer** never writes code (analysis only)
- **Copywriter** generates all textual content and alt texts (never codes)
- **Backend** and **Frontend** work in child theme only
- **Reviewer** is mandatory for all code
- **Validator** is final quality gate
- **Scripter** owns scripts, catalog, and media uploads from recursos/
